 ...
 * <li/>1: pointer to static primitive fields (if any)
...
public class JopClassInfo extends OldClassInfo implements Serializable {
	...
	private int instSize; // offset 0
	public int staticRefVarAddress; // offset 1
	private int instGCinfo; // offset 2
	...
	/**
	 * Mapping from original constant pool indices to constant pool indices used
	 * in the binary
	 */
	...
	public int setAddress(OldAppInfo ai, int addr) {
		...
		for (i = 0; i < listIT.size(); i++) {
			...
			boolean matchInterface = implementsInterface(it.meth.getCli().clazz
					.getClassName());
		...
		do {
			...
			for (int i = 0; i < interfaces.length; i++) {
				if (ifname.equals(interfaces[i])) {
					...
					JopClassInfo superCli = (JopClassInfo) appInfo.cliMap
							.get(interfaces[i]);
			...
			if (pos != -1) {
				...
				case Constants.CONSTANT_Methodref:
					...
					if (minf.getMethod().isStatic()
							||
							// <init> and privat methods are called with
							// invokespecial
	...
	public void dumpStaticFields(PrintWriter out, PrintWriter outLinkInfo,
			boolean ref) {
		...
		for (i = 0; i < clft.len; ++i) {
			if (clft.isStatic[i]) {
				if (clft.isReference[i] == ref) {
					outLinkInfo.println("static " + clazz.getClassName() + "."
							+ clft.key[i] + " " + clft.idx[i]);
					if (clft.size[i] == 1) {
						out.print("\t\t0,");
					} else {
						out.print("\t\t0, 0,");
	...
	public void dump(PrintWriter out, PrintWriter outLinkInfo) {
		...
		outLinkInfo.println("class " + clazz.getClassName() + " "
				+ methodsAddress + " " + cpoolAddress);
		...
		if (!clazz.isInterface()) {
			...
			for (i = 0; i < clvt.len; i++) {
				...
				outLinkInfo.println(" -mtab " + clvt.mi[i].getFQMethodName()
						+ " " + addr);
				addr += ClassStructConstants.METH_STR;
			}
		} else {
		...
		for (Entry<Integer, Integer> entry : cpoolMap.entrySet()) {
			outLinkInfo.println(" -constmap " + entry.getKey() + " "
					+ entry.getValue());
		}
