...
public class TransientDetectorScopeEntry implements Runnable {
	...
	public List reduceCollisionSet(final Reducer it, final List motions) {
		...
		final List ret = new LinkedList();
		System.out.print(" " + voxel_map.size()+" ");
	...
	public int determineCollisions(final List motions, List ret) {
		...
		// Motion[] _motions = (Motion[]) motions.toArray(new
		// Motion[motions.size()]);
	...
	public List createMotions() {
		...
		for (int i = 0, pos = 0; i < currentFrame.planeCnt; i++) {
			...
			final cdx.statetable.Vector3d old_pos = state.get(new CallSign(
					craft.getCallsign()));
			if (old_pos == null) {
				...
				// FIXME: There are illegal static references in the following
				// line because a change in memory area is done. This is an issue on
				// how the Memory class is implemented in JOP
	...
	// These two live in the scope of the CollisionDetectorHandler
