 ...
 * 
 * <p>
 * The null character <code>'&#92;u0000'</code> and characters in the range
 * <code>'&#92;u0080'</code> to <code>'&#92;u07FF'</code> are represented by a
 * pair of bytes:
 * 
 * <blockquote>
 * <table border="1" cellspacing="0" cellpadding="8" width="50%" * summary="Bit values and bytes">
 ...
 * rather than 1-byte, so that the encoded strings never have embedded nulls.
...
public interface DataInput {
	 ...
	 * array <code>b</code>. The number of bytes read is equal to the length of
	 * <code>b</code>.
	 * <p>
	 * This method blocks until one of the following conditions occurs:
	 * <p>
	 * <ul>
	 * <li><code>b.length</code> bytes of input data are available, in which
	 ...
	 * into element <code>b[0]</code>, the next one into <code>b[1]</code>, and
	 * so on. If an exception is thrown from this method, then it may be that
	 * some but not all bytes of <code>b</code> have been updated with data from
	 * the input stream.
	 * 
	 * @param b
	 ...
	 * zero, then no bytes are read. Otherwise, the first byte read is stored
	 * into element <code>b[off]</code>, the next one into <code>b[off+1]</code>
	 ...
	 * written by the <code>writeByte</code> method of interface
	 * <code>DataOutput</code> if the argument to <code>writeByte</code> was
	 * intended to be a value in the range <code>0</code> through
	 * <code>255</code>.
	 * 
	 ...
	 * 
	 * This method is suitable for reading the bytes written by the
	 * <code>writeShort</code> method of interface <code>DataOutput</code> if
	 * the argument to <code>writeShort</code> was intended to be a value in the
	 ...
	 * the method <code>Float.intBitsToFloat</code>. This method is suitable for
	 * reading bytes written by the <code>writeFloat</code> method of interface
	 ...
	 * this by first constructing a <code>long</code> value in exactly the
	 * manner of the <code>readlong</code> method, then converting this
	 * <code>long</code> value to a <code>double</code> in exactly the manner of
	 * the method <code>Double.longBitsToDouble</code>. This method is suitable
	 * for reading bytes written by the <code>writeDouble</code> method of
	 * interface <code>DataOutput</code>.
	 * 
	 ...
	 * integer in exactly the manner of the <code>readUnsignedShort</code>
	 * method . This integer value is called the <i>UTF length</i> and specifies
	 * the number of additional bytes to be read. These bytes are then converted
	 * to characters by considering them in groups. The length of each group is
