...
public abstract class SafeAbstractList<E> extends SafeAbstractCollection<E>
		implements List<E> {
	...
	public ListIterator<E> listIterator(final int index) {
		if (index < 0 || index > size())
			throw new IndexOutOfBoundsException(
					"Index less than zero or bigger than list size");
	...
	private class Itr implements Iterator<E> {
		...
		public void remove() {
			...
			try {
				SafeAbstractList.this.remove(lastRet);
				if (lastRet < cursor)
					cursor--;
				lastRet = -1;
	...
	private class ListItr extends Itr implements ListIterator<E> {
		...
		public void set(E e) {
			...
			try {
				SafeAbstractList.this.set(lastRet, e);
				expectedModCount = modCount;
		...
		public void add(E e) {
			...
			try {
				SafeAbstractList.this.add(cursor++, e);
				lastRet = -1;
				expectedModCount = modCount;
			} catch (IndexOutOfBoundsException ex) {
				throw concModExc;
	 ...
	 * 
	 * <p>
	 * The subclass's {@code set(int, E)}, {@code get(int)}, {@code add(int, E)}, {@code remove(int)}, {@code addAll(int,
	 ...
	 * the ArrayList by {@code (toIndex - fromIndex)} elements. (If
...
class SubList<E> extends SafeAbstractList<E> {
	private SafeAbstractList<E> l;
	private int offset;
	private int size;
	private int expectedModCount;

	SubList(SafeAbstractList<E> list, int fromIndex, int toIndex) {
		if (fromIndex < 0)
...
class RandomAccessSubList<E> extends SubList<E> implements RandomAccess {
	RandomAccessSubList(SafeAbstractList<E> list, int fromIndex, int toIndex) {
