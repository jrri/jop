...
public class ConnectionCore implements IDispose, Connection {
	...
	public SafeVector<PacketCore> packets;
//	public Queue<PacketCore> packets;
	...
	public ConnectionCore(byte packetsCapacity) {
		this.packets = new SafeVector<PacketCore>(packetsCapacity);
...
//		PacketCore packet = packets.dequeue(timeout);
		try {
			PacketCore packet = packets.remove(0);
...
//		return null;
	...
	public void send(Packet packet) {
		...
		if (ImmortalEntry.packetsToBeProcessed.size() < ImmortalEntry.packetsToBeProcessed.capacity()) {
			ImmortalEntry.packetsToBeProcessed.add(p);
		} else {
			/* Packet cannot be processed, discard it and return it to it's
			 * pool */
			ImmortalEntry.resourcePool.putPacket(p);
		}
...
//		}
	...
	public synchronized void processPacket(PacketCore packet) {
		if (isOpen && (packets.size() != packets.capacity())) {
			packets.add(packet);
		} else {
			ImmortalEntry.resourcePool.putPacket(packet);
		}
