...
import libs.examples.hijac.cdx.DetectorControl;
import libs.examples.hijac.cdx.Motion;
import libs.examples.hijac.cdx.PersistentData;
import libs.examples.hijac.cdx.RawFrame;
import libs.examples.hijac.cdx.Reducer;
...
public class ReducerHandler extends AperiodicEventHandler {
	...
	public void reduceAndPartitionMotions(Reducer reducer, List<Motion> motions) {
		...
		for (Iterator<Motion> iter = motions.iterator(); iter.hasNext();) {
			reducer.performVoxelHashing(iter.next(), voxel_map, graph_colors);
		}
