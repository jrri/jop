...
class Reducer {
   ...
   */
	protected void putIntoMap(SafeHashMap voxel_map, Vector2d voxel, Motion motion) {
  ...
  protected boolean isInVoxel(Vector2d voxel, Motion motion) {
	...
	protected void dfsVoxelHashRecurse(Motion motion, Vector2d next_voxel,
			SafeHashMap<Vector2d, ArrayList<?>> voxel_map, SafeHashMap<Vector2d, String> graph_colors) {
		...
		if (!graph_colors.containsKey(next_voxel)
			...
			// graph_colors.put(new Vector2d(next_voxel), blank.toString());

			//TODO: This generates an illegal assignment as it is not possible
			// to get scope information from string objects in JOP.
   ...
   */
	protected void performVoxelHashing(Motion motion,
			SafeHashMap<Vector2d, ArrayList<?>> voxel_map,
			SafeHashMap<Vector2d, String> graph_colors) {
  ...
  public LinkedList reduceCollisionSet(LinkedList motions) {
    ...
    SafeHashMap<Vector2d, ArrayList<?>> voxel_map = new SafeHashMap<Vector2d, ArrayList<?>>();
    SafeHashMap<Vector2d, String> graph_colors = new SafeHashMap<Vector2d, String>();
