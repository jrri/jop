...
import libs.examples.hijac.cdx.CallSign;
import libs.examples.hijac.cdx.Constants;
import libs.examples.hijac.cdx.RawFrame;
import libs.examples.hijac.cdx.StateTable;
import libs.examples.hijac.cdx.Vector3d;

import libs.safeutil.SafeHashSet;
...
public class InputFrameHandler extends PeriodicEventHandler {
	...
	public void handleAsyncEvent() {
		...
		/* This shouldn't be necessary, detectorReady is true after all handlers are registered */
...
//		Set<CallSign> vanished = mission.getState().getCallSigns();
		Set<CallSign> vanished = new SafeHashSet<CallSign>();
		for (Iterator<CallSign> iter = mission.getState().getCallSigns().iterator(); iter.hasNext();) {
			vanished.add(iter.next());
		}
...
//			final Vector3d old_pos = mission.getState().get(new CallSign(cs));
			final Vector3d old_pos = mission.getState().get(callsign);
		 ...
		 * of our model.
		 */
		for (Iterator<CallSign> iter = vanished.iterator(); iter.hasNext();) {
//			state.remove(iter.next());
			iter.next();
			iter.remove();
		}
	}
